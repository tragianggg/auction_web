...
let currentPrice = 100000; // Giá khởi điểm (VND)
let timeLeft = 3600; // Thời gian đếm ngược (giây)
...
function updateCountdown() {
    ...
    if (timeLeft < 0) {
        ...
        messageDisplay.textContent = "Đấu giá kết thúc!";
...
bidButton.addEventListener("click", () => {
    ...
    if (isNaN(bidAmount) || bidAmount <= currentPrice) {
        messageDisplay.textContent = "Giá đấu không hợp lệ!";
    } else {
        currentPrice = bidAmount;
        currentPriceDisplay.textContent = currentPrice.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' });
        messageDisplay.textContent = "Đấu giá thành công!";
    }
});
